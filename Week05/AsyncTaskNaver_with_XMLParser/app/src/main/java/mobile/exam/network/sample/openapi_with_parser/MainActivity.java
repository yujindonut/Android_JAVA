...
//        adapter = new ArrayAdapter<NaverBookDto>(this, android.R.layout.simple_list_item_1, resultList);
        adapter = new MyAdapter(this,R.layout.book_item, resultList);
...
//xml데이터 담은거 parsedList안에 넣음
            ArrayList<NaverBookDto> parseredList = parser.parse(result);     // 오픈API 결과의 파싱 수행
//            Log.i(TAG + "parser!", parseredList.toString());
            if (parseredList.size() == 0) {
                Toast.makeText(MainActivity.this, "No data!", Toast.LENGTH_SHORT).show();
            } else {
                resultList.clear();
                resultList.addAll(parseredList);
