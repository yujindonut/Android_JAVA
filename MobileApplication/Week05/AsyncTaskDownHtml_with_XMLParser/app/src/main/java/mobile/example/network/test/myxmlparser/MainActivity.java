...
//        res/values/strings.xml 의 server_url 값을 읽어옴
    ...
    class NetworkAsyncTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            if (result == null) { //값이 아예 안넘어왔을때 (이상한 result포함)
                cancel(true);
...
//          parser 생성 및 OpenAPI 수신 결과를 사용하여 parsing 수행
            ...
            resultList = parser.parse(result); //xml result를 넘겨줌 / 하나의 영화정보 dto객체 넘어옴
            Log.i(TAG, result);
            //입력값 잘못되게 입력하면 이상한 xml가 날라옴
            if (resultList == null) {       // 올바른 결과를 수신하지 못하였을 경우 안내
                ...
                //dto의 toString으로 취급됨 : notify를 내부적으로 알아서 해서, 따로 비우거나 그러지 않아도됨
            }
        }
